...
func Checksum(msg *Message) uint8 {
	return ChecksumIgnoringFields(msg, map[uint16]any{10: nil})
}

// Body length of the mesage, ignoring tags 8, 9 and 10
func BodyLength(msg *Message) uint64 {
	return BodyLengthIgnoringFields(msg, map[uint16]any{8: nil, 9: nil, 10: nil})
}

// Checksum of message, can provide custom tags that needs to be ignored
func ChecksumIgnoringFields(msg *Message, ignoreTags map[uint16]any) uint8 {
	var result int
...
func BodyLengthIgnoringFields(msg *Message, ignoreTags map[uint16]any) uint64 {
	var result uint64
	for _, field := range *msg {
		if _, ok := ignoreTags[field.Tag]; !ok {
